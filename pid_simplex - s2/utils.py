...
# ------------------------- Config knobs -------------------------
MIN_DIST   = 1e-8      # Deduplication threshold (minimum distance between nodes)
...
GAP_STOP_TOL = 1e-4     # End iteration if the optimality gap reaches this threshold
    ...
    while True:
        ...
        if use_obbt and len(first_stage_vars) > 0:
            ...
            if len(active_objs) == 0:
                ...
                # Multiple targets will cause OBBT to throw error, so we'll stop here
            ...
            # When restricting the variable set, warmstart must be turned off
            obbt_analysis(model,
